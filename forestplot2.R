...
plot.si <- as.tibble(ptable.si)[, -c(3:4, 7:8)]

plot.si <- plot.si %>%
  mutate(contrast = factor(contrast, levels = unique(contrast), 
                             labels = c("No PO Use, Some Social Support", "PY Medical Use Only, Some Social Support", 
...
### merge both tables
plot.final <- rbind(plot.si, rd_table.si, ic_table.si)
...
### the risk differences tables for the interaction effects and remove the columns SE and df
### rename columns med_po, talkprob_r and rate
plot.sa <- as.tibble(ptable.sa)[, -c(3:4, 7:8)]
...
### merge both tables
plot.final.2 <- rbind(plot.sa, rd_table.sa, ic_table.sa)
...
ggsave("interaction_suicid.jpg", width = 18, height = 8, units = "in", dpi = 1800)
ggsave("interaction_suicid.eps", width = 18, height = 8, units = "in", dpi = 1800)
...
### forestplot exposure PY Any Non-Medical Use
plot.final.nomedpo %>%
  ### definiting the x, y axis of the plot and CIs
  ggplot(aes(y = contrast, x = estimate, xmin = asymp.LCL, xmax = asymp.UCL, color = color)) +
  ### size of points for effect estimates and shapes
